...
use caliptra_api::mailbox::{
    CmRandomStirReq, MailboxReqHeader, MailboxRespHeader, MailboxRespHeaderVarSize,
...
/// MCU-local DPE response buffer, sized to fit cert chain and certify-key
/// responses without blowing the user-space stack.  The upstream
/// `InvokeDpeResp::DATA_MAX_SIZE` can be 25 KB+ which is far too large.
pub(crate) const MAX_DPE_RESP_DATA_SIZE: usize = 2048;

#[repr(C)]
#[derive(
    Debug, zerocopy::IntoBytes, zerocopy::FromBytes, zerocopy::Immutable, zerocopy::KnownLayout,
)]
pub(crate) struct DpeResp {
    pub hdr: MailboxRespHeader,
    pub data_size: u32,
    pub data: [u8; MAX_DPE_RESP_DATA_SIZE],
}

impl Default for DpeResp {
    fn default() -> Self {
        Self {
            hdr: MailboxRespHeader::default(),
            data_size: 0,
            data: [0u8; MAX_DPE_RESP_DATA_SIZE],
        }
    }
}
